...
class SingleNetwork(nn.Module):
    ...
    def forward(self, data):
        blocks = self.backbone(data) # [4,3,512,512] -> [[4, 256, 128, 128],[4, 512, 64, 64],[4, 1024, 32, 32],[4, 2048, 32, 32] ]
        v3plus_feature = self.head(blocks)      # (b, c, h, w) [4, 256, 128, 128]
        b, c, h, w = v3plus_feature.shape

        pred = self.classifier(v3plus_feature) # torch.Size([4, 21, 128, 128])

        b, c, h, w = data.shape
        pred = F.interpolate(pred, size=(h, w), mode='bilinear', align_corners=True) # torch.Size([4, 21, 512, 512])
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        # Map convolutions
        out = torch.cat([m(x) for m in self.map_convs], dim=1) # [4, 2048, 32, 32]->[4,1024,32,32]
        out = self.map_bn(out)
        out = self.leak_relu(out)       # add activation layer
        out = self.red_conv(out) # [4,256,32,32]

        # Global pooling
        pool = self._global_pooling(x) # [4, 2048, 1, 1]
        pool = self.global_pooling_conv(pool) #[4,256,1,1]
        pool = self.global_pooling_bn(pool)

        pool = self.leak_relu(pool)  # add activation layer

        pool = self.pool_red_conv(pool) #[4,256,1,1]
        if self.training or self.pooling_size is None:
            pool = pool.repeat(1, 1, x.size(2), x.size(3)) # [4, 256, 32, 32]

        out += pool
        out = self.red_bn(out)
        out = self.leak_relu(out)  # add activation layer
        return out # [4, 256, 32, 32]
...
class Head(nn.Module):
    ...
    def forward(self, f_list):
        f = f_list[-1] # [4, 2048, 32, 32]
        f = self.aspp(f) # [4, 256, 32, 32]

        low_level_features = f_list[0] # [4,256,128,128]
        low_h, low_w = low_level_features.size(2), low_level_features.size(3) # 128,128
        low_level_features = self.reduce(low_level_features) # [4, 48, 128, 128]

        f = F.interpolate(f, size=(low_h, low_w), mode='bilinear', align_corners=True) # [4, 256, 128, 128]
        f = torch.cat((f, low_level_features), dim=1) # torch.Size([4, 304, 128, 128])
        f = self.last_conv(f) # [4, 256, 128, 128]

...
class BoxMaskGenerator (MaskGenerator):
    def __init__(self, prop_range, n_boxes=1, random_aspect_ratio=True, prop_by_area=True, within_bounds=True, invert=False):
        ...
        self.prop_range = prop_range #true
        self.n_boxes = n_boxes # 3 
        self.random_aspect_ratio = random_aspect_ratio # true
        self.prop_by_area = prop_by_area # [0.25,0.5]
        self.within_bounds = within_bounds # true
        self.invert = invert # true 

    def generate_params(self, n_masks, mask_shape, rng=None): # n_masks=4 mask_shape=[512,512] 
        """
        ...
        if self.prop_by_area:
            ...
            mask_props = rng.uniform(self.prop_range[0], self.prop_range[1], size=(n_masks, self.n_boxes)) # [4,3]shape 的 0.25-0.5的数据
        ...
        sizes = np.round(np.stack([y_props, x_props], axis=2) * np.array(mask_shape)[None, None, :]) # [4,3,2] 4个mask 3个box 2代表长宽

        if self.within_bounds:
            positions = np.round((np.array(mask_shape) - sizes) * rng.uniform(low=0.0, high=1.0, size=sizes.shape)) # 坐标
            rectangles = np.append(positions, positions + sizes, axis=2) ##### [4,3,4]
        else:

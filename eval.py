...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        name = data['fn']
        if self.need_cam:
            pred,both_imgs,cam_img = self.sliding_eval(img, config.eval_crop_size, config.eval_stride_rate, device,label= label)
        else:
        ...
        if self.save_path is not None:
            ...
            cam_path= self.save_path+'_GradCAM' # gradcam++ EigenCAM
            ensure_dir(cam_path)
            ...
            if self.need_cam:
                ...
                Image.fromarray(cam_img).save( cam_path+ '/' + name + '_cam.png')
                
                for i, both_img in enumerate(both_imgs) : 
                    Image.fromarray(both_img).save( cam_path + '/' + name + '_' + str(i) + '_both.png')  
                logger.info('Save the image cam ' + fn)
                'save colored result'
                ...
                result_img.save(os.path.join(cam_path, name+'_color.png'))

                'save raw result'
                cv2.imwrite(os.path.join(cam_path, fn), pred)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        segmentor = SegEvaluator(dataset, config.num_classes, config.image_mean,
                                 ...
                                 args.show_image, need_cam =False ) # True

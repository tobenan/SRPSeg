...
def generate_class_mask_batch(pred, classes):
    pred, classes = torch.broadcast_tensors(pred.unsqueeze(1), classes.unsqueeze(2).unsqueeze(3)) # broadcast([1,512,512],[10,1,1]) -> [10,512,512],[10,512,512]
    N = pred.eq(classes).sum(1) # 若都为空，则0维度求和后从空变为0
    return N

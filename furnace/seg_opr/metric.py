...
def compute_score(hist, correct, labeled):
    ...
    mean_pixel_acc = correct / labeled # 计算有问题
    cpa = np.diag(hist)/hist.sum(0)# precison
    recall = np.diag(hist)/hist.sum(1)# precison
    mpa= np.nanmean(cpa) # 与miou类似

    return iu, mean_IU, mean_IU_no_back, mean_pixel_acc, cpa, recall, mpa

...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x) # [4, 128, 256, 256]
        x = self.bn1(x)   # [4, 128, 256, 256] float32
        x = self.relu(x)
        x = self.maxpool(x)  # [4, 128, 128, 128]

        blocks = []
        x = self.layer1(x); # [4, 256, 128, 128]
        blocks.append(x)
        x = self.layer2(x); # [4, 512, 64, 64]
        blocks.append(x)
        x = self.layer3(x); # [4, 1024, 32, 32]
        blocks.append(x)
        x = self.layer4(x); # [4, 2048, 32, 32]
        blocks.append(x)

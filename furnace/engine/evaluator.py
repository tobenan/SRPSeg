...
class Evaluator(object):
    def __init__(self, dataset, class_num, image_mean, image_std, network,
                 ...
                 verbose=False, save_path=None, show_image=False, eval_one=False, need_cam = False):
        self.eval_time = 0
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        if '.pth' in model_indice:
            ...
            model_slice = np.array(sorted_models)[bound] #  select .pth
        ...
        for model in models:
            ...
            i=0
            if not isinstance(result_line, tuple):
                result_line = [result_line]
                i=2
                
            for single_line in result_line:
                results.write('\n--------result'+str(i)+'\n')
                i+=1
                results.write(single_line)
            results.write('\n')
            results.flush()
    ...
    def single_process_evalutation(self):
        ...
        #for idx in tqdm(range(10)): # 
            dd = self.dataset[idx]
            results_list = self.func_per_iteration(dd,self.devices[0])
            if self.eval_one:
                all_results.append(results_list[0])# 只存在一个元素
            else:    
                all_results0.append(results_list[0])
                all_results1.append(results_list[1])
                all_results.append(results_list[2])# 融合元素
        ...
        result_line0 = self.compute_metric(all_results0)
        result_line1 = self.compute_metric(all_results1)
        result_line = self.compute_metric(all_results)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line0,result_line1,result_line
    ...
    def multi_process_evaluation(self):
        ...
        for _ in tqdm(range(self.ndata)):
            ...
            if self.eval_one:
                all_results.append(t[0])# 此时只存在一个元素
            else:    
                all_results0.append(t[0])
                all_results1.append(t[1])
                all_results.append(t[2])# 融合的元素
            if self.verbose:
    ...
    def sliding_eval(self, img, crop_size, stride_rate, device=None, label=None):
        ori_rows, ori_cols, c = img.shape
        processed_pred0 = np.zeros((ori_rows, ori_cols, self.class_num))
        processed_pred1 = np.zeros((ori_rows, ori_cols, self.class_num))
        ...
        for s in self.multi_scales:
            ...
            if self.need_cam:
                ...
                pred0, pred1, both_img, cam_img = self.scale_process(img_scale,
                                    (ori_rows, ori_cols),
                                    crop_size, stride_rate, device, label=label)
                processed_pred0 += pred0
                processed_pred1 += pred1
            else:
                pred0, pred1= self.scale_process(img_scale,
                                                (ori_rows, ori_cols),
                                                crop_size, stride_rate, device)
                processed_pred0 += pred0
                processed_pred1 += pred1#[h,w,c]
        predlist=np.stack([processed_pred0,processed_pred1], axis=0) # (2, 366, 500, 21) 有负值
        pred = np.mean(predlist, axis=0)
        pred_soft=pred
        pred = pred.argmax(2)
        pred0 = processed_pred0.argmax(2)
        pred1 = processed_pred1.argmax(2)
        if self.need_cam:
            return pred, pred0, pred1, pred_soft, both_img, cam_img
    ...
    def scale_process(self, img, ori_shape, crop_size, stride_rate,
        ...
        if long_size <= min(crop_size[0], crop_size[1]):
            ...
            if label is not None:
                input_data, margin, label = self.process_image(img, crop_size, label=label)     # pad image  
            else:
                input_data, margin = self.process_image(img, crop_size)     # pad image [366,500,3]
            
            if self.need_cam: 
                score, both_img, cam_img = self.val_func_process(input_data, device, label=label,margin=margin)    
            else:
                score = self.val_func_process(input_data, device)    #返回两组数据[2,1,21,512,512]
            score=torch.stack(score,dim=0)
            score=score.squeeze(1)#[2,21,512,512]
            ...
            if label is not None:
                label, margin = pad_image_to_shape(label, crop_size,
                                               cv2.BORDER_CONSTANT, value=255)# 标签用255填充
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    temp_score =torch.squeeze(score,1)#[2,21,512,512]
            ...
            score = data_scale
            score = score[:, margin[0]:(score.shape[1] - margin[1]), # 记得修改score.shape
                    margin[2]:(score.shape[2] - margin[3])]
        ######
        score0 = score[0].permute(1, 2, 0)
        score1 = score[1].permute(1, 2, 0)
        data_output0 = cv2.resize(score0.cpu().numpy(),
                                (ori_shape[1], ori_shape[0]),
                                interpolation=cv2.INTER_LINEAR) #[500,334,21]
        data_output1 = cv2.resize(score1.cpu().numpy(),
                                (ori_shape[1], ori_shape[0]),
                                interpolation=cv2.INTER_LINEAR) #[500,334,21]
        if self.need_cam:
            return  data_output0, data_output1, both_img, cam_img
        return data_output0, data_output1

    def val_func_process(self, input_data, device=None, label=None, margin=None):
        input_data = np.ascontiguousarray(input_data[None, :, :, :], 
                                          dtype=np.float32) #[1,3,512,512]
        #  使数组连续存储 system run faster
        input_data = torch.FloatTensor(input_data).cuda(device) ##

        with torch.cuda.device(input_data.get_device()):
            self.val_func.eval() # set train_state = false
            self.val_func.to(input_data.get_device())
            with torch.no_grad():
                score = self.val_func(input_data)# (2,[1,21,512,512]) 存储了两个模型的输出
                #score = score[0] # [21,512,512]
            ...
            if self.need_cam:
                if label is not None:
                    if self.is_flip:
                        print("---------label cant flip-----------")
                        raise RuntimeError   
                score1 = torch.stack(score, 0)#[2,1,21,512,512]
                score1 = score1.mean(0)[0] #[21,512,512]
                with torch.enable_grad(): ### 必须加
                    both_img, cam_img = gen_cam(self.val_func, input_data, score1, label=label, margin=margin)
                return score, both_img, cam_img    
        return score

    def process_image(self, img, crop_size=None, label=None):
        p_img = img
        ...
        if crop_size is not None:
            ...
            p_img, margin = pad_image_to_shape(p_img, crop_size,
                                               cv2.BORDER_CONSTANT, value=0) #[512,512,3]
            p_img = p_img.transpose(2, 0, 1)
            if label is not None:
                label, margin = pad_image_to_shape(label, crop_size,
                                               cv2.BORDER_CONSTANT, value=255)# 标签用255填充
                return p_img, margin, label
            return p_img, margin
